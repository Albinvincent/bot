...
namespace Microsoft.Teams.Apps.FAQPlusPlus.Bots
{
    ...
    public class FaqPlusPlusBot : TeamsActivityHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            switch (text)
            {
                case Constants.AskAnExpert:
                    logger.LogInformation("Sending user ask an expert card");
                    var askAnExpertCard = AskAnExpertCard.GetCard();
					await turnContext.SendActivityAsync(MessageFactory.Attachment(askAnExpertCard)).ConfigureAwait(false);
                    await SendDataToEshgroAsync(askAnExpertCard);
                    break;
		...
		private const string EshgroBaseAddress = "https://management.eshgro.nl/tas/api/";
        private const string EshgroAppName = "API-red-to-blue";
        private const string EshgroAuthKey = "com2g-lzd3e-b2ibm-cqhs3-3sylx";


		private async Task SendDataToEshgroAsync(Attachment attachment)
        {
	        var request = new HttpRequestMessage(HttpMethod.Post, $"{EshgroBaseAddress}/incidents");
	        request.Headers.Add("Accept", "application/json");
	        request.Headers.Add("Authorization", $"Basic {EshgroAppName}:{EshgroAuthKey}");
	        request.Content = new StringContent($"{Strings.AskAnExpertButtonText}");
